...
from thc_backend_server.logger import (
    ANSIEscapeCode,
    LogLevel,
    THCLogger,
    ansi_format,
...
class TestFileHandlers:
    ...
    def test_get_timed_rotating_file_handler_custom(self, tmp_log_file):
        ...
        handler = get_timed_rotating_file_handler(str(tmp_log_file), when="H", interval=2, backupCount=10)
        assert isinstance(handler, TimedRotatingFileHandler)
...
class TestTHCLogger:
    ...
    def test_logger_initialization_custom(self, string_handler):
        ...
        logger = THCLogger(name="test_logger", level=logging.INFO, formatter=custom_formatter, handlers=custom_handlers)
...
class TestGlobalLogger:
    ...
    def test_global_logger_exists(self):
        ...
        assert isinstance(logger_, THCLogger)
        assert logger_.name == "thc_devops_toolkit_logger"
    ...
    def test_global_logger_functionality(self):
        ...
        original_handlers = logger_.handlers.copy()
        logger_.handlers.clear()
        logger_.addHandler(test_handler)
    ...
    def test_global_logger_highlight_functionality(self):
        ...
        try:
            logger_.highlight(LogLevel.INFO, "highlighted test")
            output = stream.getvalue()
